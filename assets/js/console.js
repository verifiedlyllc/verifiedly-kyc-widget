...
const scrollView = document.querySelector('.tab_Console .Contents');
const tabs = document.querySelectorAll('.tab_console .Contents .Content');
const nav = document.querySelectorAll('.tab_console nav a');
const currentNav = document.querySelector('.tab_console nav .current');
let currentTab = 0;
...
const animateToTab = index => {
  ...
  nav[currentTab].classList.remove('console_active');
  nav[index].classList.add('console_active');
  const time = {
  ...
  const tick = now => {
    ...
    const navOffset = easeInOutCubic(time.elapsed, nav[currentTab].offsetLeft, nav[index].offsetLeft - nav[currentTab].offsetLeft, time.duration);
    const indicatorWidth = easeInOutCubic(time.elapsed, nav[currentTab].clientWidth, nav[index].clientWidth - nav[currentTab].clientWidth, time.duration);
